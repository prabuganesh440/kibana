    ...
    return {
      ...
      link: function ($scope, $el, attr) {
        ...
        $scope.$watchCollection('vis.params', prereq(function () {
          ...
          // If this is for printing report
          if ($scope.printReport) {
              // This is to take care of expanding the data-table div size
              // Explanation for the constants used
              // td height : 22px
              // th height: 26px
              // panel title height: 28.7px (vis title)
              // height between the table and panel div: 20px
              if (($scope.vis.type.title === 'Data table' || $scope.vis.type.title === 'Matrix') && $scope.esResp) {
                   var val = $scope.vis.params.perPage * 22 + 75;
                   $el[0].style.height = val + 'px';
              }
              var elements = document.getElementsByClassName('legend-ul');
        ...
        $scope.$watch('esResp', prereq(function (resp, prevResp) {
          if (!resp) return;
          $scope.renderbot.render(resp, $scope.printReport);
          // If this is for printing report
          if ($scope.printReport) {
              // This is to take care of expanding the data-table div size
              // This is to take care of expanding the data-table div size
