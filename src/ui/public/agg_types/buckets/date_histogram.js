define(function (require) {
  return function DateHistogramAggType(timefilter, config, Private) {
    ...
    function getInterval(agg) {
      ...
      // When agg type is matrix and
      // No of columns field is enabled we set the value of interval
      // based on number of columns entered by the user.
      if( agg.vis.type.name == 'matrix' && agg.vis.params.enableNoOfColumns ) {
          ...
          interval = duration._milliseconds / agg.vis.params.NofColumns;
          // set the interval value in seconds
          interval_string = interval/1000 + 's'

          // set the interval to 24 hours when the
          // collapse time headers is enabled in matrix 
          // visualization.
          if(agg.vis.params.collapseTimeHeaders) {
            interval_string = 86400 + 's';
          }
