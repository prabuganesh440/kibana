  ...
  return function DataFactory(Private) {
    ...
    Data.prototype.pieNames = function (data) {
      ...
      _.forEach(data, function (obj) {
        ...
        count = 0;
        total_size = 0
        length = obj.slices.children.length;

        _.forEach(self.getNames(obj, columns), function (name) {
            // Calculate the total size... This is required to calculate
            // percentage for each slice below..
            if ( count < length) {
                total_size = total_size + obj.slices.children[count].size;
            }
            count = count+1;
            names.push(name);
        });

        // Now update pielabel.. we are doing this calculation ourselves
        // as we are not able to find the calculated pie values. This
        // may go wrong somewhere so we need to test this with multiple
        // use cases
        count = 0;
        _.forEach(names, function(name) {
           if ( count < length) {
               percent = (obj.slices.children[count].size * 100)/total_size
               // Create a new variable pielabel with percentage value.. In
               // Legend, we will use this
               name.pielabel = name.label + ' (' + percent.toFixed(2) + '%)';
               count = count+1;
           }
        });
      });
