module.exports = async (kbnServer, server, config) => {
  ...
  server.exposeStaticDir('/vienna_images/{path*}', resolve(__dirname, 'vienna_images/'));
  ...
  server.route({
    ...
    handler: function (req, reply) {
      ...
      if (kbnServer.status.isGreen()) {
        let user_name = 'testadmin';
        let user_role = 'admin';
        let user_permission = 'admin';
        let shipper_url = req.headers.shipper_url;
        let user_home_dashboard = req.headers.home_page;
        return reply.renderApp(app, user_name, user_role, user_permission, shipper_url, user_home_dashboard);
      } else {
  ...
  server.decorate('reply', 'renderApp', function (app, user_name, user_role, user_permission, shipper_url, user_home_dashboard) {
    const payload = {
      app: app,
      nav: uiExports.apps,
      ...
      user_home_dashboard: user_home_dashboard,
