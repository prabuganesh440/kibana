  ...
  return function TabbedAggResponseWriterProvider(Private) {
    ...
    TabbedAggResponseWriter.prototype.calcSum = function (agg, value, block) {
      ...
      if (value.type === 'bucket') {
          this.calcSumRowBuffer.push(value);
      }
      ...
      if (value.type === 'bucket') {
          this.calcSumRowBuffer.pop(value);
      }
    ...
    TabbedAggResponseWriter.prototype.cell = function (agg, value, block) {
      if (this.asAggConfigResults) {
        ...
        if (this.opts.metricsInPercentage && value.type === 'metric') {
            // If '.single-term' exist in the Dict, it means we have single
            // bucketing and so get the sum from the corresponding dict
            if ('.single-term' in this.sumDict) {
                value.sum = this.sumDict['.single-term'][agg.id];
            } else {
                value.sum = this.sumDict[this.rowBuffer[0]][agg.id];
            }
        }
      }
