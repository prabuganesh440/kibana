      ...
      controller: function ($scope, $injector, $q, AppState, es) {
        ...
        $scope.bulkDelete = function () {
          if( $scope.currentTab.service.type === 'dashboard') {
             ...
             var index = dash_list.indexOf('Home');
             if ( index > -1)
             {
               alert("You cannot delete the Home dashboard. Please uncheck it and try again");
               return;
             }
          }
          ...
          {
              $scope.currentTab.service.delete($scope.selectedItems).then(refreshData).then(function () {
                  $scope.selectedItems.length = 0;
              });
          }
          else
