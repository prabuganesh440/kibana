  ...
  return function SavedObjectFactory(es, kbnIndex, Promise, Private, Notifier, safeConfirm, indexPatterns) {
    ...
    function SavedObject(config) {
      ...
      self.applyESResp = function (resp) {
        ...
        var visState = undefined;
        if (resp._type == 'visualization') {
            visState = JSON.parse(self._source.visState);
        }
        var allowedRoles = self._source.allowedRolesJSON ? JSON.parse(self._source.allowedRolesJSON) : [];
        var load_allowed = rbacCheckForModifyPermission(resp._type, self._source.title, visState, allowedRoles, ['modify','view']);

        if (!load_allowed) {
            throw new errors.SavedObjectNotAllowed('load', type, self.id);
        }
      ...
      function rbacCheckForModifyPermission(type, title, visState, allowedRoles, permission_list) {
        ...
        if (check_permission) {
            ...
            _.each(allowedRoles, function(role) {
                if (role.name == current_user[1] && _.contains(permission_list, role.permission) ){
                    oper_allowed = true;
                }
            });
        } else {
       ...
       */
      self.save = function (is_new_object) {
        ...
        var allowedRoles = self.allowedRolesJSON ? JSON.parse(self.allowedRolesJSON) : [];
        var save_allowed = rbacCheckForModifyPermission(type, self.title, visState, allowedRoles, ['modify']);

        if (!save_allowed) {
            notify.error("Operation save for " + type + " " + self.id + " is not allowed");
            throw new errors.SavedObjectNotAllowed('save', type, self.id);
        }
        ...
        // index the document
        return self.saveSource(body, is_new_object);
      };

      self.saveSource = function (source, is_new_object) {
        ...
        .then(finish)
        .catch(function (err) {
          // is_new_object is a flag which will be set to true when a new vienna object
          // is created or any vienna object is loaded and saved as another new object.
          // In such cases we confirm overwriting. In other cases we save without any
          // confirmation for overwriting.
          if (_.get(err, 'origError.status') === 409 && is_new_object == true) {
      ...
      self.delete = function () {
        ...
        // Only an admin can delete objects
        if (current_user[2] != 'admin') {
            notify.error("Operation delete for " + type + " '" + self.id + "' is not allowed");
            throw new errors.SavedObjectNotAllowed('delete', type, self.id);
        }
