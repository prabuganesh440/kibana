  ...
  return function PieChartFactory(Private) {
    ...
    PieChart.prototype.addPath = function (width, height, svg, slices) {
      ...
      var outerArc = d3.svg.arc()
          .innerRadius(function(d){return Math.max(radius+20)})
          .outerRadius(function(d){return Math.max(radius+20)});
      ...
      text.enter()
        .append('text')
          ...
          .attr("dy", ".35em")
          .text(function(d) {
              if (d.percentOfGroup === undefined) {
                  return '';
              }
             value = d.value.toFixed(2);
             var text = d.name + '(' + String(value) + '%)';
             return text;
          });
        ...
        text.transition().duration(1000)
             .attrTween("transform", function(d) {
                ...
                console.log("First interpolate");
                var interpolate = d3.interpolate(this._current, d);
                console.log("interpolate invoke 1");
                this._current = interpolate(0);
                return function(t) {
                   console.log("interpolate invoke 2");
