define(function (require) {
  ...
  return function RedirectWhenNotAllowedFn($location, kbnUrl, Notifier, Promise) {
    var SavedObjectNotAllowed = errors.SavedObjectNotAllowed;
    ...
    return function (mapping) {
      ...
      return function (err) {
        // if this error is not "NotAllowed", rethrow
        if (!(err instanceof SavedObjectNotAllowed)) throw err;

        var url = mapping[err.savedObjectType] || mapping['*'];
        if (!url) url = '/';

        url += (url.indexOf('?') >= 0 ? '&' : '?') + `notAllowed=${err.savedObjectType}`;
