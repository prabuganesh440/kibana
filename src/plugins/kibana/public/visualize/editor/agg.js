  ...
  .directive('visEditorAgg', function ($compile, $parse, $filter, Private, Notifier) {
    ...
    return {
      ...
      link: function ($scope, $el, attrs, kbnForm) {
        ...
        /**
         * Describe the pipeline aggregation, for display in the collapsed agg header
         * @return {[type]} [description]
         */
        $scope.describe_pipeline_aggregation = function () {
          if (!$scope.agg.params.pipeline_aggs) return '';
          const label = "with " + $scope.agg.params.pipeline_aggs.type;
          return label ? label : '';
        };
